...
import com.example.tracker_data.local.TrackerFoodDao
...
class TrackerRepositoryImpl(
    ...
    override suspend fun insertTrackedFood(food: TrackedFood) {
        db.insertTrackedFood(food.toTrackedFoodEntity())
    }

    override suspend fun deleteTrackedFood(food: TrackedFood) {
        db.deleteTrackedFood(food.toTrackedFoodEntity())
    }

    override fun getFoodsForDate(localDate: LocalDate): Flow<List<TrackedFood>> {
       val res = db.getFoodsForDate(day = localDate.dayOfMonth, month = localDate.monthValue, year = localDate.year)
        return res.map { entities ->
